...
### Day 31-32: ドキュメント作成（2025-12-05実施）

#### 操作マニュアル更新
- [x] 請求書管理の操作方法
  - docs/user_manual.md v2.0更新
  - 一覧表示、検索、月次自動生成、手動作成、編集、PDF出力、送付、削除の操作手順
- [x] 入金管理の操作方法
  - 一覧表示、登録、編集、削除、入金状況確認、未払い自動チェックの操作手順
- [x] 在庫管理の操作方法
  - 一覧表示、新規登録、在庫数更新、入出庫記録、在庫アラート確認、削除の操作手順
- [x] トラブルシューティング追加
  - Q7: 請求書PDF文字化け、Q8: 月次請求書生成問題、Q9: 入金ステータス未更新、Q10: 在庫アラート未表示

#### API仕様書作成（将来のAPI化に備えて）
- [x] エンドポイント一覧
  - docs/api_specification.md v1.0作成
  - 請求書API、入金API、在庫API、案件API、企業API
- [x] リクエスト/レスポンス例
  - 認証（Bearer Token）、ページネーション、日付フォーマット、エラーコード
  - 各APIのリクエスト/レスポンス例を記載

#### データ移行手順書
- [x] 既存請求書データの移行
  - docs/data_migration_guide.md v2.0更新
  - Phase 2移行手順追加（Day 1: 請求書、Day 2: 入金、Day 3: 在庫）
  - インポートスクリプト例、インポート確認手順
- [x] 在庫データの初期投入
  - 在庫データ、入出庫記録のインポート手順
  - データ検証項目（請求書、入金、在庫の検証）
- [x] 移行完了チェックリスト拡張
  - Phase 2専用チェックリスト15項目追加

**確認項目:**
- [x] 操作マニュアルが揃っている（user_manual.md v2.0）
- [x] データ移行手順が明確（data_migration_guide.md v2.0）
- [x] API仕様書が作成されている（api_specification.md v1.0）

**成果物:**
- docs/user_manual.md: v1.0 → v2.0（請求書・入金・在庫管理追加）
- docs/api_specification.md: 新規作成
- docs/data_migration_guide.md: v1.0 → v2.0（Phase 2移行手順追加）
